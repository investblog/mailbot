...
- **Форма** = приписка к сообщению о лимите/вытеснении + URL-кнопка «Открыть @allinmailbot» под
  основной клавиатурой (не отдельное сообщение). Ведёт на **бота Catchall в Telegram**, не на
  сайт: для аудитории бота это нативнее (один тап, без браузера) — решение владельца 04.09.2026.
...
- **Атрибуция** = deep link `t.me/{CATCHALL_BOT}?start=mb_{rate|evict}_{owner.id}` (owner.id в алфавите
  Telegram: `tg:42` → `tg_42`, payload ≤ 64); Catchall получает его в `/start`.
...
| `CATCHALL_BOT` | `allinmailbot` | username бота Catchall в Telegram — цель deep link промо (§7.1) |
