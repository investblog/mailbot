...
`sendMessage` — это и есть «доставка» письма. Исход `ingest()` управляет ответом источнику, чтобы не терять и не зацикливать письма:

| Ситуация | Класс | `email()` (SMTP) | `/ingest` (HTTP) | Доп. действие |
|---|---|---|---|---|
| `2xx` ok | delivered | принять | `200 ok` | инкремент `otp_caught`, промо |
| `403` бот заблокирован | permanent | принять (дроп) | `200` | **снести адреса чата** (`deleteBoxesForChat`) — доставка невозможна, сужаем abuse-поверхность |
| `400` (наша ошибка форматирования) | permanent | принять (дроп) | `200` | залогировать; ретрай не поможет |
| битый MIME (parse fail) | permanent | принять (дроп) | `200` | залогировать |
| `429` / `5xx` / сетевой сбой | transient | **`451`** (отправляющий MX ретраит) | **`503`** (релей ретраит) | — |

Принцип: транзиентное — отдаём наверх для ретрая; постоянное — принимаем и тихо дропаем (бесконечный ретрай вреднее потери одного письма).
...
- **Единство abuse-проверок:** `/ingest` (план Б) проходит ровно тот же набор проверок, что `email()` — релей не должен быть лазейкой.
...
- **Размер письма** — до 25 MiB. **IP reputation** — shared inbound CF иногда отдаёт `451`.
